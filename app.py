...
class App(tk.Tk):
    ...
    # MISC
    # Error in case "latest currency hasn't been pulled"
    error=0
    def __init__(self):
        super().__init__()
        ...
        self.latest.place(relx=.5,rely=.5,anchor='c',relwidth=1,relheight=1)
    ...
    def base_on_amount_input(self,*args):
        ...
        if self.base_amount.isnumeric():
            ...
            # If the base currency is valid,
            if self.base_currency in self.valid_entries:
                # Check if self.latest has values, i.e. "Latest" button has been pressed.
                if self.latest:
                    # Get hashmap using the integer of the input number
    ...
    def targ_on_curr_input(self,*args):
        ...
        # Remove the buttons if the input has 1 in length or none
